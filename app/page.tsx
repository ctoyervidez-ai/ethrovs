...
export default function Home() {
  ...
  return <>
    ...
    <main>
      ...
      <section className="sec wrap" id="precio"><div className="sec-head"><h2>Website Express <span className="serif">24 h.</span></h2><p className="label">{c.pricing.label}</p></div><div className="price-grid"><div className="card rv"><div className="cur" role="group" aria-label="Currency"><button type="button" aria-pressed={market === "us"} onClick={() => setMarket("us")}>USA · USD</button><button type="button" aria-pressed={market === "mx"} onClick={() => setMarket("mx")}>México · MXN</button></div><div className="amount"><span className="from">{c.pricing.from}</span><span className="fig">{pricing.price}</span><span className="cc">{pricing.currency}</span></div><p className="price-description">{c.pricing.description}</p><ul className="incl">{c.pricing.items.map((item) => <li key={item}><span className="ck" aria-hidden="true">✓</span><span>{item}</span></li>)}</ul><p className="terms">{c.pricing.terms}</p><a className="btn btn-acid btn-wide" href="https://wa.me/19569511763?text=Hola%20ETHROVS%2C%20quiero%20reservar%20mi%20p%C3%A1gina%20Express%2024H" target="_blank" rel="noreferrer">{c.pricing.cta} <span className="arw">↗</span></a></div><div className="card extras rv"><h3>{c.pricing.extrasTitle}</h3><p>{c.pricing.extrasText}</p><dl>{c.pricing.extras.map((extra, index) => <div className="ext-row" key={extra}><dt>{extra}</dt><dd>{pricing.extras[index]}</dd></div>)}</dl></div></div></section>
      ...
      <section className="sec wrap" id="contacto"><div className="cta rv"><div className="cta-in"><div><p className="label">{c.contact.label}</p><h2>{c.contact.line1} <span className="serif">{c.contact.line2}</span></h2><p>{c.contact.text}</p></div><div className="cta-actions"><a className="btn btn-acid btn-wide" href="https://wa.me/19569511763?text=Hola%20ETHROVS%2C%20quiero%20una%20p%C3%A1gina%20web" target="_blank" rel="noreferrer">{c.contact.whatsapp} <span className="arw">↗</span></a><a className="btn btn-line btn-wide" href="mailto:ethernaldevops@gmail.com?subject=Quiero%20una%20p%C3%A1gina%20web">{c.contact.email} <span className="arw">↗</span></a><p className="cta-note">{c.contact.note}</p></div></div></div></section>
    </main>
